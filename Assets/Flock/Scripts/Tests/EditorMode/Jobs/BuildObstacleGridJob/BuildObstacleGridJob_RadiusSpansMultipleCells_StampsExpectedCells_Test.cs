...
namespace Flock.Scripts.Tests.EditorMode.Jobs.BuildObstacleGridJob {
    ...
    public sealed class BuildObstacleGridJob_RadiusSpansMultipleCells_StampsExpectedCells_Test {
        [Test]
        public void BuildObstacleGridJob_RadiusSpansMultipleCells_StampsExpectedCells_Test_Run() {
            // Grid
            float3 origin = new float3(0f, 0f, 0f);
            int3 resolution = new int3(8, 8, 8);
            float cellSize = 1f;

            // Obstacle: radius spans 3 cells per axis (2..4) => 27 cells stamped.
            ...
            // Expected: x/y/z in [2..4] => 3*3*3 = 27
            int expectedMin = 2;
            int expectedMax = 4;
            int expectedCellCount = 27;
